  ...
  (let ((cc (make-instance 'compilation-unit)))
    ...
    (let ((foo (parse-into-term-representation '(foo ?x ?y) cc)))
      ...
      (destructuring-bind (x y) (arguments foo)
        ...
        (5am:is (eql '?y (name y)))))
    (let ((conj (parse-into-term-representation '(and ?x ?y) cc)))
      (5am:is-true (typep conj 'conjunction-term))
      (5am:is (eql 'and (operator conj)))
      (5am:is-true (typep (arguments conj) 'cons))
      (5am:is (= 2 (length (arguments conj))))
      (destructuring-bind (x y) (arguments conj)
        (5am:is-true (typep x 'variable-term))
        (5am:is (eql '?x (name x)))
        (5am:is-true (typep y 'variable-term))
        (5am:is (eql '?y (name y)))))
    (let ((disj (parse-into-term-representation '(or ?x ?y) cc)))
      (5am:is-true (typep disj 'disjunction-term))
      (5am:is (eql 'or (operator disj)))
      (5am:is-true (typep (arguments disj) 'cons))
      (5am:is (= 2 (length (arguments disj))))
      (destructuring-bind (x y) (arguments disj)
        (5am:is-true (typep x 'variable-term))
        (5am:is (eql '?x (name x)))
        (5am:is-true (typep y 'variable-term))
        (5am:is (eql '?y (name y)))))
    (let ((foreach (parse-into-term-representation
                    '(foreach ?x (f ?x)) cc)))
      (5am:is-true (typep foreach 'universal-quantification-term))
      (5am:is (eql 'foreach (operator foreach)))
      (destructuring-bind (xb) (bindings foreach) 
        (5am:is-true (typep xb 'binding))
        (let ((x (binding-variable xb)))
          (5am:is-true (typep x 'variable-term))
          (5am:is (eql '?x (name x)))
          (5am:is (eql nil (slot-value foreach 'bound-variables)))
          (5am:is (= 1 (length (bound-variables foreach))))
          ;; Check that we cache the result of bound-variables
          (5am:is (not (eql nil (slot-value foreach 'bound-variables))))
          (5am:is (eql x (first (bound-variables foreach)))))
        (5am:is (eql '() (binding-keywords xb)))
        (5am:is (eql cc (binding-context xb))))
      (5am:is-true (typep (arguments foreach) 'cons))
      (5am:is (= 1 (length (arguments foreach)))))))
